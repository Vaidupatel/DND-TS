...
const CheckboxComponent: React.FC<CheckboxComponentProps> = ({ childIndex, parentID, onUpdate, onRemove }) => {
    ...
    const checkboxId = `checkbox-${parentID}-${childIndex}`;
    ...
    useEffect(() => {
        const htmlString = `
            <fieldset id="${checkboxId}">
                ...
                ${options.map((option, index) => `
                    <div>
                        <input type="checkbox" id="${checkboxId}-${index}" name="${groupName}" value="${option}" ${selectedOptions.includes(option) ? 'checked' : ''} />
                        <label for="${checkboxId}-${index}">${option}</label>
                    </div>
                `).join('')}
            </fieldset>
        `;
        ...
        onUpdate(checkboxId, htmlString, cssString);
    }, [groupName, options, selectedOptions, containerStyles, labelStyles, inputStyles, checkboxId, onUpdate]);
    ...
    return (
        <fieldset id={checkboxId} style={containerStyles} onContextMenu={openContextMenu}>
            <legend style={labelStyles}>{groupName || "Checkbox Group"}</legend>
            {options.map((option, index) => (
                <div key={index}>
                    <input
                        type="checkbox"
                        ...
                        style={inputStyles}
                    />
                    <label style={inputLabelStyle} htmlFor={`${checkboxId}-${index}`}>{option}</label>
                </div>
            ))}
        </fieldset>
    );
