...
const FigureComponent: React.FC<FigureComponentProps> = ({ childIndex, parentID, depth, maxDepth = 1, onUpdate, onRemove }) => {
    ...
    const selectFigureChildren = createSelector(
        [(state: RootState) => state.figureChild, (_, droppableFigureid: string) => droppableFigureid],
        (figureChild, droppableFigureid) => figureChild[droppableFigureid] || []
    );
    const figureChildren = useSelector((state: RootState) => selectFigureChildren(state, droppableFigureid));
    ...
    const renderComponent = (name: string, index: number) => {
        if (depth >= maxDepth) {
            return (
                <div key={`${droppableFigureid}-${index}`} style={{ padding: '10px', border: '1px dashed red' }}>
    ...
    return (
        <figure
            ...
            className={droppableFigureid}
            ref={setFigureNodeRef}
            onContextMenu={openContextMenu}
        >
            {figureChildren.map((name: string, index: number) => renderComponent(name, index))}
        </figure>
    );
};
