...
const OlComponent: React.FC<OlComponentProps> = ({ childIndex, parentID, depth, maxDepth = 1, draggedItemType }) => {
    ...
    const selectOlChildren = createSelector(
        [(state: RootState) => state.olChild, (_, droppableOlid: string) => droppableOlid],
        (olChild, droppableOlid) => olChild[droppableOlid] || []
    );
    const olChildren = useSelector((state: RootState) => selectOlChildren(state, droppableOlid));
    ...
    const renderComponent = (name: string, index: number) => {
        if (depth >= maxDepth) {
            return (
                <div key={`${droppableOlid}-${index}`} style={{ padding: '10px', border: '1px dashed red' }}>
        ...
        switch (name) {
            case 'div':
                return (
                    <DivComponent
                        key={`${droppableOlid}-${index}`}
                        childIndex={index}
                        parentID={droppableOlid}
                        depth={depth + 1}
                    />
                );
            case 'span':
            ...
            case 'header':
                return <HeaderComponent key={index} childIndex={index} parentID={droppableOlid} depth={depth + 1} />;
            case 'footer':
                return <FooterComponent key={index} childIndex={index} parentID={droppableOlid} depth={depth + 1} />;
            case 'main':
                return <MainComponent key={index} childIndex={index} parentID={droppableOlid} depth={depth + 1} />;
            case 'article':
                return <ArticleComponent key={index} childIndex={index} parentID={droppableOlid} depth={depth + 1} />;
            case 'aside':
                return <AsideComponent key={index} childIndex={index} parentID={droppableOlid} depth={depth + 1} />;
            case 'nav':
            ...
            case 'form':
                return <FormComponent key={index} childIndex={index} parentID={droppableOlid} depth={depth + 1} />;
            case 'table':
                return <TableComponent key={index} childIndex={index} parentID={droppableOlid} depth={depth + 1} />;
            case 'iframe':
                return <IFrameComponent key={index} childIndex={index} parentID={droppableOlid} depth={depth + 1} />;
            case 'figure':
                return <FigureComponent key={index} childIndex={index} parentID={droppableOlid} depth={depth + 1} />;
    ...
    return (
        ...
        >
            {olChildren.map((name: string, index: number) => renderComponent(name, index))}
        </ol>
    );
};
