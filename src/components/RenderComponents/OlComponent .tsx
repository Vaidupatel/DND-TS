...
import { addOlChild, removeOlChild } from '../../store/slices/olChildSlice';
...
const OlComponent: React.FC<OlComponentProps> = ({ childIndex, parentID, depth, maxDepth = 1, onUpdate, onRemove, draggedItemType }) => {
    ...
    useEffect(() => {
        ...
        const htmlString = `<ol class="${droppableOlid}">\n${mergedChildrenHTML}</ol>`;
        ...
        cssString += createCSSRule(`.${droppableOlid}`, baseStyles);

        // Generate CSS for LI styles
        // cssString += createCSSRule(`.${droppableOlid} li`, liStyles);
        ...
        onUpdate(droppableOlid, htmlString, cssString);
    }, [baseStyles, liStyles, childrenData, droppableOlid, onUpdate]);
    ...
    const openContextMenu = (event: React.MouseEvent<HTMLUListElement>) => {
        if (event.target === event.currentTarget) {
            ...
            addLiButton.addEventListener('click', () => {
                dispatch(addOlChild({ OlId: droppableOlid, componentName: 'li' }));
                contextMenu.remove();
                currentContextMenu = null;
            });
