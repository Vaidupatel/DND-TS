...
const ArticleComponent: React.FC<ArticleComponentProps> = ({ childIndex, parentID }) => {
    const dispatch = useDispatch();
    const droppableArticaleid = `droppableArticle-${childIndex}`;

    const { isOver, setNodeRef: setNodeArticle } = useDroppable({
        id: droppableArticaleid,
    });
    ...
    const selectarticaleChildren = createSelector(
        [(state: RootState) => state.articaleChild, (_, droppableArticaleid: string) => droppableArticaleid],
        (articaleChild, droppableArticaleid) => articaleChild[droppableArticaleid] || []
    );
    const articaleChildren = useSelector((state: RootState) => selectarticaleChildren(state, droppableArticaleid));
    const renderComponent = (name: string, index: number) => {
        switch (name) {
            case 'div':
                return <DivComponent key={index} childIndex={index} parentID={droppableArticaleid} />;
            case 'span':
                return <SpanComponent key={index} childIndex={index} parentID={droppableArticaleid} />;
            case 'section':
                return <SectionComponent key={index} childIndex={index} parentID={droppableArticaleid} />;
            case 'header':
                return <HeaderComponent key={index} childIndex={index} parentID={droppableArticaleid} />;
            case 'footer':
                return <FooterComponent key={index} childIndex={index} parentID={droppableArticaleid} />;
            case 'main':
                return <MainComponent key={index} childIndex={index} parentID={droppableArticaleid} />;
            // case 'article':
            //     return <ArticleComponent key={index} childIndex={index} parentID={droppableArticaleid} />;
            case 'aside':
                return <AsideComponent key={index} childIndex={index} parentID={droppableArticaleid} />;
            case 'nav':
                return <NavComponent key={index} childIndex={index} parentID={droppableArticaleid} />;
            // case 'ul':
    ...
    return (
        <article
            ref={setNodeArticle}
            className={`article-component-${childIndex}`}
            style={combinedArticleStyles}
            onContextMenu={openContextMenu}
        >
            {`Article : \n  Child index: ${childIndex} \n parent id = ${parentID}`}
            {articaleChildren.map((name: string, index: number) => renderComponent(name, index))}
