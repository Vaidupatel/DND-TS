...
const FormComponent: React.FC<FormComponentProps> = ({ childIndex, parentID, depth, maxDepth = 1 }) => {
    ...
    const selectFormChildren = createSelector(
        [(state: RootState) => state.formChild, (_, droppableFormid: string) => droppableFormid],
        (formChild, droppableFormid) => formChild[droppableFormid] || []
    );
    const formChildren = useSelector((state: RootState) => selectFormChildren(state, droppableFormid));
    ...
    const renderComponent = (name: string, index: number) => {
        if (depth >= maxDepth) {
            return (
                <div key={`${droppableFormid}-${index}`} style={{ padding: '10px', border: '1px dashed red' }}>
